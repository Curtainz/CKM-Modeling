...
# 读取数据
data_id <- read_xlsx("D:/GitHub/CKM-Modeling/processedData/data_need_co.xlsx")

# 将data_id中的id重命名为ID
data_id <- data_id %>% rename(ID = id)

# 去除所有year=1的行
data_id <- data_id %>% filter(year != 1)

# 删除year和stage列
data_id <- data_id %>% select(-year, -stage)

# 读取2011年数据
data <- read.csv("D:/GitHub/CKM-Modeling/ProcessedData/CHARLS-2011/Completed/7-final_cleaned_data.csv")

# 选择需要的列
data <- data %>% select(ID, gender, region, age, education)

# 更新年龄(age = age + 4)，重命名education为education_11
data$age <- data$age + 4
data <- data %>% rename(education_11 = education)

# 根据ID对data和data_id取交集
data_id <- merge(data_id, data, by = "ID")
...
household_roster <- data %>% select(ID, householdID, communityID, ba000_w2_3, )
