...
# 读取体检数据
biodata <- read_dta("D:/R/R_Data/2025.3.7/Charls2011_basic/biomarkers.dta")
...
# 读取健康状况数据
healthdata <- read_dta("D:/R/R_Data/2025.3.7/Charls2011_basic/health_status_and_functioning.dta")

# 从healthdata中提取<个人ID, 家庭ID, 社区ID, 高血压(1=yes, 2=no, 后同), 高血压再确认, 糖尿病, 心脏病, 肾病>
extracted_healthdata <- healthdata %>% select(ID, householdID, communityID, da007_1_, da008_1_, da007_3_, da007_7_, da007_9_)
...
# 读取血检数据
blooddata <- read_dta("D:/R/R_Data/2025.3.7/Charls2011_basic/Blood_20140429.dta")
...
# 剔除<身高, 腰围, 血检四项数据>缺失值
cleaned_data_1 <- merged_data_2 %>% filter(!is.na(qi002) & !is.na(qm002) & !is.na(newglu) & !is.na(newhba1c) & !is.na(newhdl) & !is.na(newtg))

# 导出数据
write_dta(cleaned_data_1, "D:/R/R_Data/2025.3.7/Charls2011_basic/cleaned_data.dta")
